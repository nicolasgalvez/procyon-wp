...
## Project
- **Key:** `PWC`
- **Board:** https://procyoncreative.atlassian.net/jira/software/c/projects/PWC/boards/234
...
## Workflow Rules
- **No work without a ticket.** Every branch must reference a `PWC-NNN` Jira ticket. If one doesn't exist, create it first via `/jira-ticket create` (or in the Jira UI) before opening the branch.
- **Branch name format:** `PWC-NNN-short-description` (e.g. `PWC-42-add-login-form`).
- **Ticket requirements:**
